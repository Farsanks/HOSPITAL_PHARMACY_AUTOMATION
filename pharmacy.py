...
class Window1:
    ...
    def _init_(self, master):
        ...
        self.btnlogin = Button(self.login2, text="Login", width=17, font=("arial", 20, "bold"),
        ...
        self.btnReg = Button(self.login3, text="Patients Registerations", font=("arial", 20, "bold"), state=DISABLED,
                             command=self.Registeration_window)
        self.btnReg.grid(row=0, column=0, pady=8, padx=57)

        self.btnPhar = Button(self.login3, text="Pharmacy Management", font=("arial", 20, "bold"), state=DISABLED,
                              command=self.Pharmacy_window)
...
class Window2:
    ...
    def _init_(self, master):
        ...
        def iResetRecord():
            iResetRecord = tkinter.messagebox.askokcancel("Patient Registrations",
        ...
        MembersName_F = LabelFrame(FrameDetails, bd=10, width=350, height=400, font=('arial', 12, 'bold'),
                                   text="Patient Name ", relief=RIDGE)
        ...
        user_info = {"Firstname": Firstname, "Surname": Surname, "Address": Address, "Postcode": Postcode,
                     "Telephone": Telephone}  # ........DICTIONARY
        counter = 1
        for i in user_info:
            self.cur_entrybox = "entry" + i
            self.cur_entrybox = Entry(MembersName_F, font=('arial', 14, 'bold'), bd=7, insertwidth=2,
                                      textvariable=user_info[i])
        ...
        '', 'Pilot Licence', 'Driving Licence', 'Passport', 'Student ID')  # .............. Array
        self.cboProve_of_ID.current(0)
        ...
        '', 'Full Member', 'Annual membership', 'Pay As You Go', 'Homorary Member')  # ............ Array
        self.cboType_of_Member.current(0)
        ...
        '', 'Visa Card', 'Master Card', 'Debit card', 'Cash')  # ................. Array
        self.cboMethod_of_Payment.current(0)
        ...
        self.chkMembership = Checkbutton(MembersName_F, text="Fees", variable=var4, onvalue=1, offvalue=0,
                                         font=('arial', 16, 'bold'), command=Fees).grid(row=10, column=0, sticky=W)
        ...
        # ------------------------------------Recipt ----------------------------------
        self.lblLable = Label(Recipt_ButtonFrame, font=('arial', 10, 'bold'), pady=10,
                              text="Member Ref\t Firstname\t Surename\t Address\t\t Date Reg.\t Telephone\t Member Paid",
                              bd=7)
        ...
        # ------------------------------------Buttons-----------------------------
        self.btnRecipt = Button(Recipt_ButtonFrame, padx=18, bd=7, font=('arial', 10, 'bold'), width=13, text="Receipt",
                                command=Receipt).grid(row=2, column=0)

        self.btnReset = Button(Recipt_ButtonFrame, padx=18, bd=7, font=('arial', 10, 'bold'), width=13, text="Reset",
                               command=Reset).grid(row=2, column=1)

        self.btnExit = Button(Recipt_ButtonFrame, padx=18, bd=7, font=('arial', 10, 'bold'), width=13, text="Exit",
                              command=iExit).grid(row=2, column=2)

        self.btnDelete = Button(Recipt_ButtonFrame, padx=18, text="Delete", font=("arial", 10, "bold"), width=13, bd=7,
                                command=iDelete).grid(row=2, column=3)
    ...
    def _init_(self, master):
        ...
        def iReceipt():
            ...
            self.txtFrameDetail.insert(END,
                                       cmbTablets.get() + "\t\t" + RefNo.get() + "\t" + Dose.get() + "\t\t" + NoofTablets.get() + "\t" + Lot.get() + "\t" + IssueDate.get() +
        ...
        DataFrameLEFT = LabelFrame(DataFrame, bd=10, width=800, height=300, font=("arial", 12, "bold"),
                                   text="Patient Detail:", relief=RIDGE, padx=20)
        DataFrameLEFT.pack(side=LEFT)

        DataFrameRIGHT = LabelFrame(DataFrame, bd=10, width=450, height=300, font=("arial", 12, "bold"),
                                    text="Prescription:", relief=RIDGE, padx=20)
        ...
        self.Tablet = ttk.Combobox(DataFrameLEFT, font=("arial", 12, "bold"), state="readonly", width=23,
                                   textvariable=cmbTablets)
        ...
        entry_box = {"Patient Name:": PatientName, "DateOfBirth:": DateOfBirth, "NHS No:": NHSNo,
                     "Patient Address:": PatientAddress, "Patient ID:": PatientID, "Lot:": Lot
        ...
        labels = ["Dose:", "Issue Date:", "Exp Date:", "Side Effects:", "Ref No:", "Storage Advice:", "Using Machine:",
        ...
        entry_box = {"Dose:": Dose, "Issue Date:": IssueDate, "Exp Date:": ExpDate, "Side Effects:": SideEffects,
                     "Ref No:": RefNo,
                     "Storage Advice:": StorageAdvice, "Using Machine:": UsingMachine,
                     "Further Information:": FurtherInformation}  # ........DICTIONARY
        ...
        self.lblLabel = Label(FrameDetail, font=("arial", 10, "bold"), pady=8,
                              text="Name of Tablet \tReference No. \tDoseage \tNo.of Tablets \tLot \tIssue Date \tExp. Date \tStrength\tStorage Adv. \tNHS Number \tPatient Name\t DOB\t Address", )
